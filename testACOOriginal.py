...
def main():
    ...
    print("3. Configurando algoritmo de Colonia de Hormigas (ACO original)...")
    ...
    print("4. Ejecutando optimización ACO original... (Por favor espera)")
    inicio = time.time()
    ...
    with open('horario_solucion_aco_original.csv', 'w', newline='', encoding='utf-8') as archivo_csv:
    ...
    print("-" * 35)
    print("El horario se ha guardado en 'horario_solucion_aco_original.csv'")

    # --- GENERAR Y GUARDAR GRÁFICA DE CONVERGENCIA ---
    print("\n5. Generando gráfica de convergencia...")
    plt.figure(figsize=(10, 6))
    plt.plot(historial, color='g', linewidth=2)
    plt.title('Curva de Convergencia - Optimización por Colonia de Hormigas (ACO Original)', fontsize=14)
    plt.xlabel('Generaciones', fontsize=12)
    plt.ylabel('Costo de la Función Objetivo', fontsize=12)
    plt.grid(True, linestyle='--', alpha=0.7)
    plt.tight_layout()
    plt.savefig("convergencia_ACO_original.png", dpi=300)
    print("Gráfica guardada como 'convergencia_ACO_original.png' en la raíz del proyecto.")
